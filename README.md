...
`UNWIRED_LABS_TOKEN` and `UNWIRED_LABS_ENDPOINT` reserve the server-side credentials for the planned sequential fallback provider. The token is loaded only from the Hub environment and must never be included in normalized telemetry or MQTT messages.
