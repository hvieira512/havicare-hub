...
final class ModelController
{
    ...
    {
        return $this->json->result($this->service->list((string)$request->getUri()->getQuery(), RequestContext::baseUrl($request)));
    }

    public function filters(ServerRequestInterface $request): Response
    {
        return $this->json->result($this->service->filters());
    }

    public function deviceTypeSuppliersModels(ServerRequestInterface $request): Response
    {
        return $this->json->result($this->service->deviceTypeSuppliersModels(RequestContext::baseUrl($request)));
    }

    public function template(ServerRequestInterface $request): Response
    {
        return $this->json->result($this->service->template((string)$request->getUri()->getQuery()));
    }

    public function show(array $params, ServerRequestInterface $request): Response
    {
        return $this->json->result($this->service->show((int)$params['id'], RequestContext::baseUrl($request)));
    }
    ...
    public function create(ServerRequestInterface $request): Response
    {
        ...
        return $this->json->result($this->service->create($payload, $request->getUploadedFiles()['image'] ?? null));
    }
