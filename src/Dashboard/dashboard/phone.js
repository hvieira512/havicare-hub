...
export function normalizePhoneControl(control) {
    ...
    const countryCode =
        control.querySelector("[data-phone-country]")?.value || DEFAULT_COUNTRY;
    const localDigits = phoneDigits(
        control.querySelector("[data-phone-local]")?.value || "",
    );
    if (localDigits === "") {
        return "";
    }
...
export function syncPhoneControl(target) {
    const control = target.matches?.("[data-phone-control]")
        ? target
        : target.closest("[data-phone-control]");
    if (!control) return;
